...
ts.filter = {
	...
	buildRow: function(table, c, wo) {
		...
		for (column = 0; column < columns; column++) {
			...
			// CUSTOM CODE: Only allows filters to be rendered for $header which
			// has the "filterable" class.
	...
	checkFilters: function(table, filter, skipFirst) {
		...
		if (c.showProcessing) {
			...
			ts.filter.findRows(table, filters, combinedFilters);
			// Filter was performed, now we need to update the count tag's value (if present)
			var $table = $(table);
			var count_selector = $table.attr('filterable-count-container');
			if(count_selector) {
				var $count_container = $(count_selector);
				var row_count = $table.find('tbody tr:visible').length;
				$count_container.html(row_count);
			}
			return false;
		}
	},
