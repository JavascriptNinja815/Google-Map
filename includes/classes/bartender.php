...
class BarTender {
	...
	public function generateToReturn($label_jobs, $label_processor = False) {
		return $this->_generate($label_jobs, $label_processor);
	}

	public function generateToFile($label_jobs, $filename, $label_processor = False) {
		$xml = $this->_generate($label_jobs, $label_processor);
		$file_pointer = fopen($filename, 'wb');
		fwrite($file_pointer, $xml);
		fclose($file_pointer);
	}

	private function _generate($label_jobs, $label_processor) {
		$this->label_job_count = 0;
		$xml = '<?xml version="1.0" encoding="utf-8"?>' . "\r\n";
		$xml .= '<XMLScript Version="2.0">' . "\r\n";
		foreach($label_jobs as $label_job) {
			if(is_callable($label_processor)) {
				$label_job = $label_processor($label_job);
			}
