...
$grab_shipvias = $db->query("
	...
	WHERE
		RTRIM(LTRIM(sycrlst.ruleid)) = 'SHIPVIA'
		AND
		RTRIM(LTRIM(sycrlst.compid)) = " . $db->quote($session->login['compid']) . "
	ORDER BY
		sycrlst.descrip
");
...
foreach($grab_states as $state) {
	if(!array_key_exists(trim($state['country_alpha3']), $states)) {
		$states[trim($state['country_alpha3'])] = [];
	}
	$states[trim($state['country_alpha3'])][] = [
		'state_id' => trim($state['state_id']),
		'state' => trim($state['state']),
		'alpha2' => trim($state['alpha2'])
	];
}
...
<form method="POST" id="edit-address-container" action="<?php print BASE_URI;?>/dashboard/clients/details/addresses/<?php print empty($_POST['cshipno']) == 'add' ? 'create' : 'save';?>">
	...
	<?php
	if(!empty($_POST['cshipno'])) {
		?><input type="hidden" name="cshipno" value="<?php print htmlentities($_POST['cshipno'], ENT_QUOTES);?>" /><?php
	}
	?>
		...
		<tbody>
			...
			<tr>
				...
				<td class="value">
					<select name="shipvia">
						<option value=""></option>
						<?php
						$current_shipvia = trim($address['shipvia']);
						$shipvias = [];
						foreach($grab_shipvias as $shipvia) {
							$shipvias[] = trim($shipvia['chrvl']);
							?><option value="<?php print htmlentities(trim($shipvia['chrvl']), ENT_QUOTES);?>" <?php print $current_shipvia == trim($shipvia['chrvl']) ? 'selected' : Null;?>><?php print htmlentities(trim($shipvia['chrvl']));?></option><?php
						}
						if(!empty($current_shipvia) && !in_array($current_shipvia, $shipvias)) {
							?><option value="<?php print htmlentities($current_shipvia, ENT_QUOTES);?>" selected><?php print htmlentities($current_shipvia);?></option><?php
						}
						?>
					</select>
				</td>
				...
				<td class="value">
					<select name="country"><?php
						foreach($grab_countries as $country) {
							?><option value="<?php print $country['alpha3'];?>" <?php print $country['alpha3'] == trim($address['country']) ? 'selected' : Null;?>><?php print $country['country'];?></option><?php
						}
					?></select>
				</td>
			</tr>
			...
			<tr>
				...
				<td class="value states-container"><?php
					if(array_key_exists(trim($address['country']), $states)) {
						?><select name="state"><?php
							foreach($states[trim($address['country'])] as $state) {
								?><option value="<?php print htmlentities($state['alpha2'], ENT_QUOTES);?>" <?php print $state['alpha2'] == trim($address['state']) ? 'selected' : Null;?>><?php print htmlentities($state['state']);?></option><?php
							}
						?></select><?php
					} else {
...
<script type="text/javascript">
	...
	$(document).on('submit', 'form#edit-address-container', function(event) {
		...
		var $submit_address_overlay = activateOverlayZ(
			...
			function(response) { // Success Callback
				if(response.success) {
					// Close the "add/edit" overlay now that it's been successfully submitted.
					$form.closest('.overlayz').remove();
					// Click the "Addresses" tab to force re-loading of addresses to include added/updated address.
					$('#client-details-container .tabs .tab[page="addresses"]').click();
	...
	$(document).on('change', 'form#edit-address-container select[name="country"]', function(event) {
		...
		if(states[country] !== undefined) {
			...
			$.each(states[country], function(index, iter_state) {
				$input.append(
					$('<option>').val(iter_state['alpha2']).text(iter_state['state'])
				);
			});
		} else {
			// Use text input.
