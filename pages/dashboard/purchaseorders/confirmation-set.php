...
// Ensure Purchase Order Number has been specified.
...
// Ensure type (whether "price" or "delivery") has been specified.
if(empty($_POST['type'])) {
	print json_encode(array(
		'success' => False,
...
// Ensure value has been specified.
...
$note = "Note: " . ucwords($_POST['type']) . " Confirmation set to " . $_POST['value'] . " by " . $session->login['initials'] . " on " . $note_datetime->format('m/d/y h:i:s A') . "\r\n";
...
if($_POST['type'] == 'price') {
	$db->query("
		UPDATE
			" . DB_SCHEMA_ERP . ".pomast
		SET
			cnf_price = " . $db->quote($_POST['value']) . ",
			notes = " . $db->quote($notes) . "
		WHERE
			RTRIM(LTRIM(pomast.purno)) = " . $db->quote($_POST['purchase-order-number']) . "
	");
