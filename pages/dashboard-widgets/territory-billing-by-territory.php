...
$grab_territory_sales = $db->query("
	WITH
		this_month AS (
			...
			WHERE
				arcust.terr = ".$db->quote($_POST['territory'])."
				AND
...
$result = array(
	'success' => True,
	'territory-sales' => array()
);

foreach($grab_territory_sales as $territory_sales) {
	$territory_sales['sales-thismonth-formatted'] = number_format($territory_sales['sales_thismonth'], 2);
	$territory_sales['sales-thisyear-formatted'] = number_format($territory_sales['sales_thisyear'], 2);
	$territory_sales['sales-lastyear-formatted'] = number_format($territory_sales['sales_lastyear'], 2);
	$territory_sales['dollar-change-formatted'] = number_format($territory_sales['dollar_change'], 2);
	$territory_sales['percent-change-formatted'] = number_format($territory_sales['percent_change'], 1);
	$territory_sales['open_orders'] = number_format($territory_sales['open_orders'], 0);
	$result['territory-sales'][] = $territory_sales;
}
